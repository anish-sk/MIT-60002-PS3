...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        self.width=width
        self.height=height
        self.tile=[[0 for x in range(height)] for y in range(width)]
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        """
        self.tile[math.floor(pos.x)][math.floor(pos.y)]=max(self.tile[math.floor(pos.x)][math.floor(pos.y)]-capacity,0)
    ...
    def is_position_in_room(self, pos):
        ...
        """
        if pos.x < self.width and pos.y < self.height and pos.x >= 0.0 and pos.y >= 0.0:
            return True
        else:
            return False
...
class EmptyRoom(RectangularRoom):
    ...
    def get_random_position(self):
        ...
        """
        return Position(float(self.width)*random.random(), float(self.height)*random.random())
...
class FaultyRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        """
        if FaultyRobot.gets_faulty(self):
            self.direction=float(360)*random.random()
        else:
            new_pos=self.position.get_new_position(self.direction,self.speed)
            if self.room.is_position_valid(new_pos):
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    for i in range(num_trials):
        ...
        for i in range(num_robots):
            robot.append(robot_type(room, speed, capacity))
        while(room.get_num_cleaned_tiles()/room.get_num_tiles() < min_coverage):
            for i in range(num_robots):
                robot[i].update_position_and_clean()
            t+=1
        time.append(t)
    
    return sum(time)/num_trials
